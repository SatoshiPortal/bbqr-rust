# test that the data capacity is correct
...
for vi, version in enumerate(rust_data_capacity):
    for ei, error_correction in enumerate(version):
        for mi, mode in enumerate(error_correction):
            
            assert mode == rust_data_capacity[vi][ei][mi]
            r = rust_data_capacity[vi][ei][mi]

            pvi = vi + 1
            pei = error_dict[ei]
            pmi = mode_dict[mi]
            assert r == data_capacity[pvi][pei][pmi]
